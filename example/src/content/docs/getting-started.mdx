...
<form id="contact-form" novalidate>
  ...
  <p id="contact-email-error" aria-live="polite"></p>
...
const emailError = document.querySelector<HTMLElement>('#contact-email-error')!
contact.subscribe(state => {
  emailError.textContent = state.errors.email?.join(' ') ?? ''
  const button = contact.element?.querySelector<HTMLButtonElement>('button[type="submit"]')
  if (button) button.disabled = state.isSubmitting
})
```
...
- Tracks the async handler through `state.isSubmitting` until its promise settles.
- Invalid submissions are always intercepted — the browser never navigates with bad data.

For a complete error summary, `aria-describedby`, and focus-management example, continue with [Validation and accessible errors](/forms/guides/validation-and-errors/).

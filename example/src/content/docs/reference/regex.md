---
title: regex
description: Named regular expressions and default error messages from the optional @samline/formatter peer.
template: doc
sidebar:
  order: 5
---
...
`regex` is exported by [`@samline/formatter`](https://github.com/samline/formatter), **not** by `@samline/forms`. It only exists when the `@samline/formatter` peer is installed in your project. The forms package does not bundle it, ship it, or declare it as a hard dependency — it is an optional peer. Importing it without installing the peer will throw a module-not-found error at build / runtime, and this page is only meaningful when the peer is on disk.
:::
...
When the value fails the pattern, the controller pushes `regex.email.errorMessage` into `state.errors.email` and applies the `css-error` attribute to the field — see [Configuration](/forms/reference/configuration/#validators) and [CSS styling](/forms/reference/css-styling/).
...
See [`validateFieldValue`](/forms/reference/api/#validatefieldvaluefield-value-rules-values) for the pure helper signature.

### Spread `regex` into a browser global alongside `browser`

The vanilla entrypoint exposes a `browser` singleton with `form`, `newForm`, `destroyForm`, and `available`. The most common pattern is to spread it together with `regex` into a single project global — see the [Browser registry helpers](/forms/getting-started/#browser-registry-helpers-bundler) section in the getting-started guide:
...
- **No types are shipped by `@samline/forms` for `regex`.** The shim declares the type as `Record<string, { pattern: RegExp; errorMessage: string }>` for projects that do not yet have the peer on disk, but the real types come from the installed peer at compile time.
...
- [`validators`](/forms/reference/configuration/#validators) — the option that consumes `regex.<key>.pattern`.
- [`validateFieldValue`](/forms/reference/api/#validatefieldvaluefield-value-rules-values) — the pure helper that runs the `pattern` rule.
- [`format`](/forms/reference/api/#formatconfig) — also depends on `@samline/formatter` being installed.

...
## Recipe: Confirm a password with `sameAs`
...
`@samline/formatter` is optional in module builds. When it is missing, the module instance logs one cached `console.error` and asynchronously restores affected fields. The standalone global IIFE already bundles formatter behavior.
:::
...
- Both names are first-class in the controller's API. `getValue('phone')` returns the raw, `getValue('phone_displayed')` returns the formatted, and watchers can target either name. See [The mirror convention](/forms/guides/formatting/#the-mirror-convention) for the full contract.
...
See [Formatting inputs](/forms/guides/formatting/) for readiness timing, prefilled values, cleanup, and distribution differences.

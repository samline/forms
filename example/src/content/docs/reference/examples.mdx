...
`@samline/formatter` is optional. When it is not installed, `format()` and `formatAll()` log a single `console.error` describing the missing dependency and return the controller unchanged. The rest of the form keeps working — no exceptions are thrown.
:::
